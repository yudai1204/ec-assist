let isDsellerPageLoading = false;

if (
  location.href.includes("dseller.net/accounts") &&
  ...
  const interval = setInterval(function () {
    if (refreshIcon.length == 0) {
      ...
      openGetDialog();
      document
        .getElementById("auctownGetStart")
        .addEventListener("click", function () {
          getStarting("auctown");
        });
      document
        .getElementById("yahooStoreGetStart")
        .addEventListener("click", function () {
          getStarting("yahooAuctionStore");
        });
    }
    console.log("loading...");
  }, 300);
  ...
  function getStarting(csvOutputMode) {
    const generalData = {
      ...
      auctownNyusatsuHyokaSegen: document.getElementById(
        "auctownNyusatsuHyokaSegen"
      ).value,
      auctownWaruiHyokaSegen: document.getElementById("auctownWaruiHyokaSegen")
        .value,
    ...
    if (confirm("取得を開始しますか？")) {
      ...
      // 1ページ目のデータを取得
      // 2ページ目以降は内部で呼び出す
      getCSV("files/category.csv").then(afterGetCsv);

      function getCSV(filename) {
        isDsellerPageLoading = true;
        ...
        document.getElementById(
          "nowGettingData"
        ).innerHTML = `${page}ページ目ファイル展開中（画面が固まります）`;
      ...
      function afterGetCsv(r) {
        ...
        logArea.value += `${now.getHours()}:${now.getMinutes()}:${now.getSeconds()} > データ一覧を取得しました データ数:${
          dataRows.length
        }\n`;
        const dataArray = [];
        const sources = [];
        for (let i = 0; i < dataRows.length; i++) {
          logArea.value += `${now.getHours()}:${now.getMinutes()}:${now.getSeconds()} > データ取得中: ${
            i + 1
          }/${dataRows.length}\n`;
          document.getElementById(
            "nowGettingData"
          ).innerHTML = `個別データ取得中: ${i + 1}/${dataRows.length}`;
          ...
          if (category[0]) {
            indiData.categoryName = category[0].innerHTML
              .slice(1, -1)
              .replace(/&gt;/g, ">");
            indiData.categoryId = translateID(indiData.categoryName);
            indiData.itemName =
              category[0].parentNode.nextElementSibling.innerHTML.replace(
                /[!-\/:-@\[-\{-~]/g,
                function (s) {
            ...
            indiData.asin =
              amazonBtn.length > 0
                ? amazonBtn[0].parentNode.parentNode
                    .getAttribute("href")
                    .replace("https://amazon.co.jp/dp/", "")
                : null;
            ...
            //個別ページにAjax
            const ajaxUrl = td[7].firstElementChild.firstElementChild
            ...
            $.ajax({
              ...
              .done(function (data) {
                // 通信成功時の処理を記述
                indiData.explain = $(data)
                  .find("#yahaucproduct_description")
                  .val()
                  .replace(/\n/g, "")
                ...
                for (let j = 0; ; j++) {
                  if ($(data).find(`#image${j}`).length) {
                    if (
                      $(data).find(`#image${j} img`).prop("naturalWidth") >
                      $(data).find(`#image${j} img`).prop("naturalHeight") * 2.4
                    ) {
                      continue;
                    }
                    if (
                      !indiData.imgURL.includes(
                        $(data).find(`#image${j} img`).attr("src")
                      )
                    ) {
                      indiData.imgURL.push(
                        $(data).find(`#image${j} img`).attr("src")
                      );
                    }
        ...
        //CSV生成
        generateCSVImpl(dataArray, csvOutputMode, generalData, sources);

        // 2ページ目は5秒後に取得
        const nextButton = document.querySelector(
          ".paginate_button#tb_product_next"
        );
        const isButtonDisabled = nextButton?.classList.contains("disabled");

        if (isButtonDisabled || !nextButton) {
          console.log("2ページ目以降のデータがないため、取得を終了します");
          clearInterval(loadingInterval);
          const now = new Date();
          logArea.value += `${now.getHours()}:${now.getMinutes()}:${now.getSeconds()} > 取得を終了しました\n`;
          document.getElementById("nowGettingData").innerHTML = "取得完了";
          ...
          const waitingIntervalForDownload = setInterval(() => {
            if (isDsellerPageLoading) {
              console.log("ZIP取得待ち。。。");
              return;
            }
            clearInterval(waitingIntervalForDownload);
            nextButton.click();
            setTimeout(() => {
              getCSV("files/category.csv").then(afterGetCsv);
            }, 4000);
  ...
  const generateCSVImpl = (dataArray, csvOutputMode, generalData, sources) => {
    const page = document.querySelector(".paginate_button.active")?.innerText;
    chrome.storage.local.get(
      {
        //<ITEM_NAME> <ITEM_EXPLAIN> <ASIN>
      ...
      function (items) {
        ...
        if (csvOutputMode === "yahooAuctionStore") {
          ...
          for (const indiData of dataArray) {
            const explain = items.explainFormat
              .replace(/<ITEM_NAME>/g, indiData.itemName)
              .replace(/<ITEM_EXPLAIN>/g, indiData.explain)
              .replace(/<ASIN>/g, ``)
              .replace(/＜/g, "<")
              .replace(/＞/g, ">")
              .replace(/／/g, "/");
            csvdata += `\n${indiData.asin},${
              indiData.categoryId
            },${indiData.itemName.slice(0, 64)},${explain.replace(
              /\n| /g,
              ""
            )},,${indiData.price},${
              Number(indiData.price) + Number(items.sokketuFee)
            },${generalData.num},${generalData.holdingPeriod},0,${
              generalData.prefecture
            },,${generalData.postage},代金${
              generalData.paymentTiming
            },${generalData.state
              .replace("新品", "未使用")
              .replace(/中古|その他/g, "全体的に状態が悪い")},`;
            //画像挿入
            for (let i = 0; i < 10; i++) {
              csvdata += `${
                indiData.imgURL.length > i
                  ? indiData.imgURL[i]
                      .slice(indiData.imgURL[i].lastIndexOf("/") + 1)
                      .replace(/\+/g, "plus")
                      .replace(/-/g, "minus")
                  : ""
              },,`;
            ...
            csvdata += `${
              generalData.auctownNyusatsuHyokaSegen === "はい" ? 0 : -5
            },${generalData.auctownWaruiHyokaSegen},${
              generalData.auctownNinshoSegen
            },いいえ,${generalData.auctownSaishuppin},,,,0,はい,,,,,,${
              generalData.folder
            },${generalData.haisouGroup},${generalData.hassoudate}`;
          }
        } else {
          //オークタウン
          csvdata =
          ...
          for (const indiData of dataArray) {
            const explain = items.explainFormat
              ...
              .replace(
                /<ASIN>/g,
                `<P>店舗用商品ID:${caesarShift(indiData.asin, 2)}</P>`
              )
              .replace(/＜/g, "<")
              .replace(/＞/g, ">")
              .replace(/／/g, "/");
            csvdata += `\n${indiData.categoryId},${indiData.itemName.slice(
              0,
              64
            )},${explain.replace(/\n| /g, "")},${indiData.price},${
              indiData.price
        ...
        document.getElementById(
          "nowGettingData"
        ).innerHTML = `ZIPファイル生成中 ( ${page}ページ目 )`;
        downloadImages(csvdata, sources, page);
      }
    );
  };
...
// 画像の一括ダウンロード
async function downloadImages(csvdata, sources, page) {
  //CSVのバイナリデータを作る
  let blob = new Blob(
    [
      new Uint8Array(
        Encoding.convert(Encoding.stringToCode(csvdata), "sjis", "unicode")
      ),
    ],
    { type: "text/csv" }
  );
  ...
  const now = new Date();
  const logArea = document.getElementById("auctownLog");
  logArea.value += `${now.getHours()}:${now.getMinutes()}:${now.getSeconds()} > ${page}ページ目画像ダウンロード中\n`;
  ...
  const imagePromises = sources.map(
    (src, i) =>
      new Promise((resolve, reject) => {
        ...
        xhr.onload = function () {
          // ファイル名とデータ返却
          const fileName = src.includes("blob:https://dseller.net/")
            ? "dseller.csv"
            : src
                .slice(src.lastIndexOf("/") + 1)
                .replace(/\+/g, "plus")
                .replace(/-/g, "minus");
  ...
  generateImagesZip(images, page);
}

// zip ファイルで画像をダウンロード
function generateImagesZip(images, page) {
  const now = new Date();
  let zip = new JSZip();

  // フォルダ作成
  const folderName = `download_${page}`;
  ...
  zip.generateAsync({ type: "blob" }).then((blob) => {
    ...
    //終了処理
    const logArea = document.getElementById("auctownLog");
    logArea.value += `${now.getHours()}:${now.getMinutes()}:${now.getSeconds()} > ${page}ページ目ZIPダウンロード中\n`;
    document.getElementById(
      "nowGettingData"
    ).innerHTML = `${page}ページ目ZIPダウンロード中`;
    isDsellerPageLoading = false;
  });
}
